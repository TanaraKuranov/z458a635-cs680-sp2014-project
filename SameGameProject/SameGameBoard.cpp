#include "StdAfx.h"
#include "SameGameBoard.h"

SameGameBoard::SameGameBoard(void)
: m_arrBoard(NULL),
m_nColumns(15), m_nRows(15),
m_nHeight(35), m_nWidth(35),
m_nRemaining(0)
{
	m_arrColors[0] = RGB(  0,  0,  0);
	m_arrColors[1] = RGB(255,  0,  0);
	m_arrColors[2] = RGB(  0,255,  0);
	m_arrColors[3] = RGB(  0,  0,255);
}

SameGameBoard::~SameGameBoard(void)
{
	DeleteBoard();
}

void SameGameBoard::SetupBoard(void)
{
	// Create board, if needed
	if(m_arrBoard == NULL)
		CreateBoard();
	// Randomly set spaces to colors
	for(int row = 0; row < m_nRows; row++)
		for(int col = 0; col < m_nColumns; col++)
			m_arrBoard[row][col] = (rand() % 3) + 1;
	// Set remaining blocks
	m_nRemaining = m_nRows * m_nColumns;
}

COLORREF SameGameBoard::GetSpaceColor(int row, int col)
{
	// Check array boundaries
	if(row < 0 || row >= m_nRows || col < 0 || col >= m_nColumns)
		return m_arrColors[0];
	return m_arrColors[m_arrBoard[row][col]];
}

void SameGameBoard::DeleteBoard(void)
{
	// Don't delete if there's no board to begin with
	if(m_arrBoard != NULL)
	{
		for(int row = 0; row < m_nRows; row++)
		{
			if(m_arrBoard[row] != NULL)
			{
				// Delete each row
				delete [] m_arrBoard[row];
				m_arrBoard[row] = NULL;
			}
		}
		// Delete row array
		delete [] m_arrBoard;
		m_arrBoard = NULL;
	}
}

void SameGameBoard::CreateBoard(void)
{
	// Check if we have a board. If so, delete it.
	if(m_arrBoard != NULL)
		DeleteBoard();
	// Create array of rows
	m_arrBoard = new int*[m_nRows];
	// Create each row
	for(int row = 0; row < m_nRows; row++)
	{
		m_arrBoard[row] = new int [m_nColumns];
		// Set spaces to empty
		for(int col = 0; col < m_nColumns; col++)
			m_arrBoard[row][col] = 0;
	}
}

int SameGameBoard::DeleteBlocks(int row, int col)
{
	// Make sure the click is valid
	if(row < 0 || row >= m_nRows || col < 0 || col >= m_nColumns)
		return -1;
	// Make sure click is not on background blocks
	int nColor = m_arrBoard[row][col];
	if(nColor == 0)
		return -1;
	// Check that we don't have a solitary block
	int nCount = -1;
	if((row - 1 >= 0 && m_arrBoard[row - 1][col] == nColor) || 
		(row + 1 < m_nRows && m_arrBoard[row + 1][col] == nColor) || 
		(col - 1 >= 0 && m_arrBoard[row][col - 1] == nColor) || 
		(col + 1 < m_nColumns && m_arrBoard[row][col + 1] == nColor))
	{
		m_arrBoard[row][col] = 0;
		nCount = 1;
		// Now, the recursive functions
		// Recursive call for up
		nCount +=
			DeleteAdjacentBlocks(row - 1, col, nColor, DIRECTION_DOWN);
		// Recursive call for down
		nCount +=
			DeleteAdjacentBlocks(row + 1, col, nColor, DIRECTION_UP);
		// Recursive call for left
		nCount +=
			DeleteAdjacentBlocks(row, col - 1, nColor, DIRECTION_RIGHT);
		// Recursive call for right
		nCount +=
			DeleteAdjacentBlocks(row, col + 1, nColor, DIRECTION_LEFT);
		// Then compact the board
		CompactBoard();
		// Remove the count from the number of blocks remaining
		m_nRemaining -= nCount;
	}
	// Return the number of blocks deleted
	return nCount;
}

int SameGameBoard::DeleteAdjacentBlocks(int row, int col, int color,
										Direction direction)
{
	// Check if location is valid
	if(row < 0 || row >= m_nRows || col < 0 || col >= m_nColumns)
		return 0;
	// Check if color is the same
	if(m_arrBoard[row][col] != color)
		return 0;
	int nCount = 1;
	m_arrBoard[row][col] = 0;
	// Now check each direction, assuming we didn't come from that way
	if(direction != DIRECTION_UP)
		nCount +=
			DeleteAdjacentBlocks(row - 1, col, color, DIRECTION_DOWN);
	if(direction != DIRECTION_DOWN)
		nCount +=
			DeleteAdjacentBlocks(row + 1, col, color, DIRECTION_UP);
	if(direction != DIRECTION_LEFT)
		nCount +=
			DeleteAdjacentBlocks(row, col - 1, color, DIRECTION_RIGHT);
	if(direction != DIRECTION_RIGHT)
		nCount +=
			DeleteAdjacentBlocks(row, col + 1, color, DIRECTION_LEFT);
	// Return total blocks removed
	return nCount;
}

void SameGameBoard::CompactBoard(void)
{
	// First, we move everything downwards
	for (int col = 0; col < m_nColumns; col++)
	{
		int nNextEmptyRow = m_nRows - 1;
		int nNextOccupiedRow = nNextEmptyRow;
		while(nNextOccupiedRow >= 0 && nNextEmptyRow >= 0)
		{
			// Find the next empty row
			while(nNextEmptyRow >= 0 &&
				m_arrBoard[nNextEmptyRow][col] != 0)
				nNextEmptyRow--;
			if(nNextEmptyRow >= 0)
			{
				// Then find the next occupied row from the empty row
				nNextOccupiedRow = nNextEmptyRow - 1;
				while(nNextOccupiedRow >= 0 &&
					m_arrBoard[nNextOccupiedRow][col] == 0)
					nNextOccupiedRow--;
				if(nNextOccupiedRow >= 0)
				{
					// Now move block from occupied to empty
					m_arrBoard[nNextEmptyRow][col] = 
						m_arrBoard[nNextOccupiedRow][col];
					m_arrBoard[nNextOccupiedRow][col] = 0;
				}
			}
		}
	}
	// Then move columns from right to left
	int nNextEmptyCol = 0;
	int nNextOccupiedCol = nNextEmptyCol;
	while(nNextEmptyCol < m_nColumns && nNextOccupiedCol < m_nColumns)
	{
		// Find next empty column
		while(nNextEmptyCol < m_nColumns && 
			m_arrBoard[m_nRows - 1][nNextEmptyCol] != 0)
			nNextEmptyCol++;
		if(nNextEmptyCol < m_nColumns)
		{
			// Find next occupied column
			nNextOccupiedCol = nNextEmptyCol + 1;
			while(nNextOccupiedCol < m_nColumns &&
				m_arrBoard[m_nRows - 1][nNextOccupiedCol] == 0)
				nNextOccupiedCol++;
			if(nNextOccupiedCol < m_nColumns)
			{
				// Move column to left
				for (int row = 0; row < m_nRows; row++)
				{
					m_arrBoard[row][nNextEmptyCol] = 
						m_arrBoard[row][nNextOccupiedCol];
					m_arrBoard[row][nNextOccupiedCol] = 0;
				}
			}
		}
	}
}

bool SameGameBoard::IsGameOver(void) const
{
	// Check each column, left to right
	for(int col = 0; col < m_nColumns; col++)
	{
		// Row by row, bottom to top
		for(int row = m_nRows - 1; row >= 0; row--)
		{
			int nColor = m_arrBoard[row][col];
			if(nColor == 0)
				break;
			else
			{
				// Check up and right
				if(row - 1 >= 0 && 
					m_arrBoard[row - 1][col] == nColor)
					return false;
				else if(col + 1 < m_nColumns && 
					m_arrBoard[row][col + 1] == nColor)
					return false;
			}
		}
	}
	// If we get here, no moves left!
	return true;
}